...
async def _run_claude(
    ...
    # The CLI reports failures on *stdout* as JSON — "Not logged in · Please run
    # /login" is the common one — while exiting non-zero with an empty stderr.
    # So parse stdout first and treat the exit code as a corroborating signal,
    # not as the first branch: checking it first discards the only useful message.
    ...
    try:
        ...
        if proc.returncode not in (0, None):
            detail = stderr_text or stdout_text or f"exited with code {proc.returncode}"
            raise HTTPException(status_code=502, detail=f"claude CLI error: {detail}")
        # Fallback: treat raw stdout as plain text
        return stdout_text, 0, 0

    if result.get("is_error") or proc.returncode not in (0, None):
        detail = result.get("result") or stderr_text or "unknown error"
        raise HTTPException(status_code=502, detail=f"claude CLI error: {detail}")
